...
-- === Terminal ===
hl.bind(mainMod .. " + return", hl.dsp.exec_cmd("uwsm app -- " .. terminal .. " --class Kitty-Main"),
  { description = "Open the Terminal", submap_universal = true })
hl.bind(mainMod .. " + s", hl.dsp.exec_cmd("uwsm app -- " .. terminal .. " --class Tmux-Main tms"),
  { description = "Open Kitty with Tmux Session", submap_universal = true })
hl.bind(mainMod .. " + SHIFT + return", hl.dsp.exec_cmd("uwsm app -- " .. terminal .. " --class Kitty-float"),
  { description = "Open the Terminal", submap_universal = true })
hl.bind(mainMod .. " + ALT + d",
  hl.dsp.exec_cmd('hyprctl dispatch exec "uwsm-app -- zen-twilight -P Media --name zen-twilight-media --no-remote"'),
  { description = "Open Zen Browser media profile", submap_universal = true })

-- === Applications ===
hl.bind(mainMod .. " + semicolon", hl.dsp.exec_cmd(shared_scripts .. "/wrapper/minimize.sh"),
...
hl.bind(mainMod .. " + c", hl.dsp.exec_cmd("uwsm app -- qalculate-qt"),
  { description = "Open Calculator", submap_universal = true })
hl.bind(mainMod .. " + m", hl.dsp.exec_cmd("uwsm app -- proton-pass"),
  { description = "Open Proton Pass", submap_universal = true })
hl.bind(mainMod .. " + r", hl.dsp.exec_cmd('rofi -show drun -run-command "uwsm app -- {cmd}"'),
...
hl.bind(mainMod .. " + ALT + v", hl.dsp.workspace.toggle_special("vesktop"),
...
local wsKeys = { "plus", "bracketleft", "braceleft", "parenleft", "ampersand",
                 "equal", "parenright", "braceright", "bracketright", "asterisk" }
...
hl.bind(mainMod .. " + SHIFT + h", hl.dsp.window.swap({ direction = "l" }), { description = "Swap current window with the window to the left" })
hl.bind(mainMod .. " + SHIFT + l", hl.dsp.window.swap({ direction = "r" }), { description = "Swap current window with the window to the right" })
hl.bind(mainMod .. " + SHIFT + j", hl.dsp.window.swap({ direction = "u" }), { description = "Swap current window with the lower window" })
hl.bind(mainMod .. " + SHIFT + k", hl.dsp.window.swap({ direction = "d" }), { description = "Swap current window with the upper window" })

hl.bind(mainMod .. " + CTRL + h", hl.dsp.window.resize({ x = -10, y = 0,  relative = true }), { description = "Resize vertical by -10",     repeating = true })
hl.bind(mainMod .. " + CTRL + l", hl.dsp.window.resize({ x = 10,  y = 0,  relative = true }), { description = "Resize vertically by 10",     repeating = true })
hl.bind(mainMod .. " + CTRL + j", hl.dsp.window.resize({ x = 0,   y = -10, relative = true }), { description = "Resize horizontally by -10", repeating = true })
hl.bind(mainMod .. " + CTRL + k", hl.dsp.window.resize({ x = 0,   y = 10,  relative = true }), { description = "Resize horizontally by 10",  repeating = true })

-- === Mouse bindings ===
hl.bind(mainMod .. " + ALT + mouse:272", hl.dsp.window.drag(),
  { description = "Move a window with left click", submap_universal = true, mouse = true })
hl.bind(mainMod .. " + ALT + mouse:273", hl.dsp.window.resize(),
  { description = "Resize a window with right click", submap_universal = true, mouse = true })
hl.bind(mainMod .. " + ALT + m", hl.dsp.exec_cmd(hyprland_scripts .. "/minimize.sh --toggle"),
  { description = "Minimize Window", submap_universal = true })

-- === Utility ===
hl.bind(mainMod .. " + PRINT", hl.dsp.exec_cmd(shared_scripts .. "/wrapper/hyprshot.sh --window"),
  { description = "Screenshot current window", submap_universal = true })
hl.bind("PRINT", hl.dsp.exec_cmd(shared_scripts .. "/wrapper/hyprshot.sh --output"),
  { description = "Screenshot current output", submap_universal = true })
hl.bind(mainMod .. " + SHIFT + PRINT", hl.dsp.exec_cmd(shared_scripts .. "/wrapper/hyprshot.sh --region"),
...
hl.bind(mainMod .. " + ALT + l", hl.dsp.exec_cmd("hyprlock -c ~/.config/hypr-chezmoi/hyprlock.conf"),
