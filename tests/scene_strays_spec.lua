...
t.describe("strays.decide", function()
  ...
  t.it("a floating stray gets a sensible box on its monitor", function()
    local width, height = strays.fit_size({ width = 1920, height = 1080 })
    t.eq(1280, width)
    t.eq(860, height)
  end)

  t.it("does not grow into an ultrawide: a dialog has a maximum useful size", function()
    local width, height = strays.fit_size({ width = 5120, height = 1440 })
    t.eq(1280, width)
    t.eq(860, height)
  end)

  t.it("still fits a small or narrow monitor by fraction, not by the cap", function()
    local width, height = strays.fit_size({ width = 800, height = 1280 })
    t.eq(560, width)
    t.eq(860, height)
  end)
end)
