...
| arrange  | (1) `compile.emit` turns every scene into global class rules (`set always` / `barred`). (2) The scene layout places every tile in `recalculate`: declared blocks by share (an ungrouped block's own windows stacked within it), strays dividing the remainder — a `"float"` scene's stray is floated for real by the open-time executor (`hypr/scene/strays.lua` + `hypr/events/scene.lua`, LEO-367), so it drops out of `recalculate`'s tiled targets once that dispatch lands; only the moment before it lands does this layout still see it, and treats it like a `"slot"` stray. (3) The corrective engine is still wired: on events it arms `schedule` → `model.intent` → `actuator`, which uses focus-dance dispatchers. (4) `layout_opts` rewrites scrolling's own options on `open_early` / `workspace.active`; the scene layout owns single-tile framing directly (`solo_extra`, no separate rewrite). | `hypr/scene/compile.lua:16` (called from `hypr/windowrules.lua:448`); `hypr/scene/provider.lua:86`, `hypr/scene/layout.lua` (`boxes`, `stack`, `floats_strays`); `hypr/scene/strays.lua`, `hypr/events/scene.lua` (group executor + stray executor); `hypr/scene/schedule.lua:106`, `hypr/scene/model.lua:339`, `hypr/scene/actuator.lua:112`; `hypr/events/layout_opts.lua:158` | Group rules are global, so a class groups on any workspace. The corrective engine and the layout fight over geometry. No event is logged for a placed stray or a placed block window yet.                                                    |
