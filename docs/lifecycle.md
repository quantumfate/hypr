...
| arrange  | (1) `compile.emit` turns every scene into global class rules (`set always` / `barred`). (2) The scene layout places every tile in `recalculate`: declared blocks by share (an ungrouped block's own windows stacked within it), strays dividing the remainder — a `"float"` scene's stray is floated for real by the open-time executor (`hypr/scene/strays.lua` + `hypr/events/scene.lua`, LEO-367), so it drops out of `recalculate`'s tiled targets once that dispatch lands; only the moment before it lands does this layout still see it, and treats it like a `"slot"` stray. (3) The corrective engine is still wired: on events it arms `schedule` → `model.intent` → `actuator`, which uses focus-dance dispatchers. (4) `layout_opts` rewrites scrolling's own options on `open_early` / `workspace.active`; the scene layout owns single-tile centring directly (`paired_fraction`, no separate rewrite). | `hypr/scene/compile.lua:16` (called from `hypr/windowrules.lua:448`); `hypr/scene/provider.lua:86`, `hypr/scene/layout.lua` (`boxes`, `stack`, `floats_strays`); `hypr/scene/strays.lua`, `hypr/events/scene.lua` (group executor + stray executor); `hypr/scene/schedule.lua:106`, `hypr/scene/model.lua:339`, `hypr/scene/actuator.lua:112`; `hypr/events/layout_opts.lua:158` | Group rules are global, so a class groups on any workspace. The corrective engine and the layout fight over geometry. No event is logged for a placed stray or a placed block window yet.                                                    |
...
| leave    | `registry.forget` runs on close. Companions reconverge across every scene that spawns one. A closing window still lists in `hl.get_windows()` while its own close event fires (spiked live), so the close path counts the leaving member gone by excluding its address. The corrective engine is re-armed. Mode withdrawal holds windows, and a later mode restores them.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `hypr/events/scene.lua:153`; `hypr/hyprfocus/hold.lua:89`                                                                                                                                                                                                                                                                                                                        | Scenes have no teardown. Cooperative veto (hyprfocus phase 3) is not wired for windows. `leave.closed`/`leave.moved` are logged.                                                                                                             |

**Logging today (LEO-352):** `hypr/lib/trace.lua` writes one systemd journal
entry per decision (`SYSLOG_IDENTIFIER=hyprfocus`, `TRACE`/`STAGE`/`EVENT`/...
as structured fields — the sink Part B decides, no JSONL file), wired at the
decision points that already exist:

| Where                                                                  | Logs                                                                                            |
| ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `hypr/events/scene.lua`, `window.open`                                 | `identify.matched` / `identify.unmatched` — the same class-match `scene_for` already does today |
| `hypr/events/scene.lua`, companion spawn/close (`converge_companions`) | `interact.companion_spawn` / `interact.companion_close`                                         |
| `hypr/events/scene.lua`, `window.close`                                | `leave.closed`                                                                                  |
| `hypr/events/scene.lua`, `window.move_to_workspace`                    | `leave.moved`                                                                                   |
| `hypr/hyprfocus/init.lua`, `M.apply` (mode apply)                      | `admit.workspace_admitted` / `admit.workspace_withdrawn` / `admit.workspace_refused` (occupied) |
| `hypr/hyprfocus/init.lua`, `M.apply_bindings` (binding admit)          | `interact.binding_withheld`                                                                     |

`,hyprfocus log --trace <address> [--follow]` wraps `journalctl --user -t
hyprfocus -o json`, and the `logs` workspace's `h` entry follows it live (see
[bin/Readme.md](../bin/Readme.md)). This is still today's mechanisms logged
where they already decide — it is not the pure `identify`/`route` stage split
Part B describes; that split is LEO-354/LEO-353/LEO-359's work, which will
move the call sites, not add a first one.
...
| 7   | `events/solo_gaps.lua` + `layout_opts.lua`          | arrange          | **Retired.** `solo_gaps.lua` is gone; framing is a branch inside the scene layout. `layout_opts.lua` now only carries scrolling's own options. | LEO-357          |
...
held window records its origin scene and returns there. The hold workspace is
silent: windows on it cannot take focus or respond to activation requests, and
if the special workspace ever becomes visible it is put away automatically.
After every mode apply the reachability invariant is checked and violations are
logged as `admit/unreachable` ([scenes.md](scenes.md#reachability-invariant)).
