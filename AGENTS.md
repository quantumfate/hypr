...
6. [docs/live-config.md](docs/live-config.md) — what the compositor watches/re-executes on a reload, what our engine caches, and what a store/code edit needs to reach a running desk.
7. [bin/Readme.md](bin/Readme.md) — the shell helpers the desktop spawns, and the state/IPC seams to the quickshell sibling repo.
8. [system-config/docs/bindings.md](../system-config/docs/bindings.md) — the
...
9. Cross-repo: `system-config/docs/hyprfocus.md` (the full `hyprfocus` engine)
...
- The same is true of a tag added by a runtime `hl.dispatch(hl.dsp.window.tag(...))` call on an already-mapped window: a compile-time rule matched on that tag never re-evaluates once the window has opened (spiked live, LEO-364). Consume such a tag from Lua reading `w.tags` synchronously in the same event pass instead (`hypr/scene/identify.lua`), never from a rule chained off it.
- `hl.dsp.window.close("address:x")` ignores its string argument and closes the **focused** window: the Lua plugin pushes a nil window upval for a non-table arg, and the nil selector falls back to focus (`Actions::xtract`). The table form `hl.dsp.window.close({ window = "address:x" })` is the one that closes the named window (read from the plugin source; the same failure bit `,proj.sh kill` and two e2e scenarios live). Window-arg dispatchers in `hl.dsp.window.*` derive their selector only from a table's `window` field — pass `{ window = ... }`, never a bare selector string.
